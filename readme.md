...
| 이름   | 내용                                                         |
| ------ | ------------------------------------------------------------ |
| 이진행 | - Jpa를 잘 알고 쓰지 않았다는 것을 깨닫았습니다. 특히 영속성 문제에서 애를 먹었습니다. 어떨때, fetch 타입을 Eager 또는 Lazy로 써야하는지 알게되었습니다.<br />- 검색기능을 개발할 때, QueryDsl을 활용하여 동적쿼리를 이용하는 것이 편하다는 것을 알게되었습니다. jpa 레파지토리에서 만들어주는 것도 좋지만, QueryDsl 문을 작성하는 것이 명시적으로 어떤 쿼리를 보내는지 알 수 있어서 좋다는 생각이 들었습니다.  <br />- CaseCade 문제를 만났습니다. Jpa가 단방향 매핑일 경우에 자식 객체의 id값을 null 로 변경한 후, orpenRemoval은 자식 객체중 외래키가 null인 것을 삭제하는 것이었습니다. db의 외래키 제약조건을 Not Null로 설정했기에, 오류가 계속 생겼습니다. 단방향 매핑일 경우에는 Not null 조건을 걸면 안되는 것을 알게되었습니다. <br />- 깃 플로우 전략은 대규모 프로젝트를 진행할 때 좋다고 들었습니다. 릴리스 브랜치를 사용하지 않고 개발을 진행하니 계층 구분도 명확하니 컨플릭트가 크게 없다는 생각이 들었습니다. 소규모 개발에서도 깃 플로우 전략이 마냥 나쁜 것은 아니라는 생각을 했습니다.<br />- 개발을 진행할 때, 컨벤션을 어기는 경우가 많이 생겼습니다. 이를 명확히 계속 해야할지, 효율을 위해서라면 어겨도 될 지 생각을 해봐야 할 것 같습니다. |
| 이진혁 |                                                              |
| 박승연 |                                                              |
| 허진영 |                                                              |
